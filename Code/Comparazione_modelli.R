...
# Set up control function per training
...
dt_pred_conf <- predict(weighted_fit, testset)
testset_copy = testset
levels(testset_copy$fetal_health) <- c("first_class", "second_class", "third_class")
result = confusionMatrix(dt_pred, testset_copy[,c("fetal_health")], mode = "prec_recall")
result

res = roc_curve(weighted_fit)
roc_res_svm_weight = res$roc_res
plot_roc_df = res$plot_roc_df
plot_roc_curve(plot_roc_df)



# Build weighted model DECISION TREEE
weighted_fit_dec_tree <- train(fetal_health ~ .,
                               data = trainset,
                               method = "rpart",
                               #verbose = FALSE,
                               weights = model_weights,
                               metric = "ROC",
                               trControl = ctrl)
weighted_fit_dec_tree

pred_conf_dec_tree <- predict(weighted_fit_dec_tree, testset)
result = confusionMatrix(pred_conf_dec_tree, testset_copy[,c("fetal_health")], mode = "prec_recall")
result


res = roc_curve(weighted_fit_dec_tree)
...
#compare tra modelli 
cv.values = resamples(list(svm=weighted_fit, rpart = weighted_fit_dec_tree))
summary(cv.values)
...
dotplot(cv.values, metric = "Kappa")
splom(cv.values,metric="Kappa")
